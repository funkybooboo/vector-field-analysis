...
void OpenMpStreamlineSolver::computeTimeStep(Field::Grid& grid) {
    ...
    const std::size_t totalCells = static_cast<std::size_t>(rowCount) * static_cast<std::size_t>(colCount);
    ...
    std::vector<std::vector<Field::Path>> localPathsCollection(threadCount_ > 0 ? threadCount_ : std::thread::hardware_concurrency());
...
#pragma omp single
        ...
        const std::size_t startIdx = static_cast<std::size_t>(tid) * segmentsPerThread + std::min(static_cast<std::size_t>(tid), remainderSegments);
        const std::size_t endIdx = startIdx + segmentsPerThread + (static_cast<std::size_t>(tid) < remainderSegments ? 1 : 0);

        if (startIdx < endIdx) {
            std::size_t currentStart = startIdx;
            
            // Adjust currentStart so we don't start in the middle of a segment owned by previous thread.
            if (tid > 0) {
                while (currentStart < endIdx && roots[indices[currentStart]] == roots[indices[currentStart - 1]]) {
                    currentStart++;
                }
            }
            ...
            while (currentStart < endIdx) {
                ...
                while (segmentEnd < totalCells && roots[indices[segmentEnd]] == roots[indices[currentStart]]) {
                    segmentEnd++;
                }
